...
class SegDNN:
  def __init__(self, vocab_size, embed_size, skip_window):
    ...
    self.w3 = tf.Variable(
      tf.random_uniform([self.tags_count, self.h], -1.0 / math.sqrt(self.h), 1.0 / math.sqrt(self.h), dtype=tf.float32),
  ...
  def train_sentence(self, sentence, tags, length):
    ...
    current_tags = self.viterbi_fd(sentence_scores, A_val, init_A_val)  # 当前参数下的最优路径
  ...
  def update_params(self, sen_matrix, embeds, embed_index, update_length):
    ...
    for i in range(update_length):
      ...
      grad = self.sess.run(self.update_embed, feed_dict={self.x: embed,
  ...
  def viterbi_fd(self, emission, A, init_A, return_score=False):
    ...
    path_score = np.ones([4, length], dtype=np.float32) * np.finfo('f').min
    ...
    if return_score:
      # return np.insert(path[max_index, 1:],[length-1],max_index), path_score[max_index, :]
      return corr_path, path_score[max_index, :]
    else:
      # return np.insert(path[max_index, 1:],[length-1],max_index)
      return corr_path
  ...
  def seg(self, sentence, model_path='tmp/model0.ckpt'):
    ...
    with tf.Session() as sess:
      ...
      # print(A_val)
      # print(init_A_val)
      current_tags = self.viterbi_fd(sentence_scores, A_val, init_A_val)
